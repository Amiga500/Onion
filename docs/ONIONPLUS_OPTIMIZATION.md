...
[![assertions](https://img.shields.io/badge/assertions-71%2C408-success?style=flat-square)](#-5-testing)
...
> 🔁 **Self-reference.** Docs commits sit inside the range they measure. The last
> **code** landmarks are [`fbd26d06`](https://github.com/Amiga500/Onion/commit/fbd26d06)
> and [`bf3deb8e`](https://github.com/Amiga500/Onion/commit/bf3deb8e). Compare with
> `OnionUI/Onion:main` at `07505ea5`, not with a sibling branch.
>
> 📚 §1 still lists the original 56-commit window through `82fab865` (long-branch SHAs,
> not present on compact). Later waves (AdvanceMENU, review 3, Flip, OnionUI-parity)
> are in the README timeline, [§4.10](#410-miyoo-mini-flip-port-921155e8),
> [§4.11](#411-onionui-parity-review), [§4.12](#412-independent-review-2026-09-01-findings-ag),
> and [§4.13](#413-2026-09-09-robcodedev-ports--review-fixes).
...
| Icon | Meaning |
|:---:|:---|
| 📏 | **OnionPlus vs `OnionUI/Onion:main`** — speedup of this fork’s path against the upstream equivalent at `07505ea5`. |
...
| ❌ | **not taken** — exists elsewhere (`v4.5-dev` or a sibling experiment) and is absent from OnionPlus on purpose. Never counted in the totals. |

> ⚠️ A 📏 figure is the **OnionPlus vs OnionUI/Onion:main** delta for that path.
> It has not been re-timed on a physical Miyoo Mini / Mini+ / Flip in this tree.

The [Summary Overview](#-summary-overview) additionally carries an **impact heat scale** in its
first column, used only for ordering:

| 🎚️ | Tier | What lands here |
|:---:|:---|:---|
| 🟥 | **Order-of-magnitude** | vectorised pixel paths, busy-wait removal, quadratic → linear, subprocess elimination in a hot loop |
| 🟧 | **Structural** | syscall / shell-out / scan eliminations with a bounded but repeatable saving |
| 🟨 | **Incremental** | smaller structural wins — binary size, one fewer copy, capped loops |
| 🟦 | **Robustness** | correctness, memory-safety and API-contract fixes — **no performance claim** |
| 🟩 | **Quality floor** | test suites, assertions and build entry points added by the port |
...
5. [Security & Hardening](#️-4-security--hardening) (incl. [§4.10 Flip](#410-miyoo-mini-flip-port-921155e8), [§4.11 parity](#411-onionui-parity-review), [§4.12 A–G](#412-independent-review-2026-09-01-findings-ag), [§4.13 2026-09-09](#413-2026-09-09-robcodedev-ports--review-fixes))
6. [Testing](#-5-testing)
7. [Build & Tooling](#️-6-build--tooling)
8. [Overall Statistics](#-7-overall-statistics)
9. [Intentionally not taken](#-8-intentionally-not-taken)
...
> Delta of OnionPlus versus `OnionUI/Onion:main` (`07505ea5`): NEON pixel conversions
> with scalar fallback, crash/memory hardening, host unit-test suite, power/CPU work
> (OSD busy-wait, brightness cache, charging cache, batmon, SQLite, overlay fork+exec),
> Flip device support from `v4.5-dev` without merging that branch, an OnionUI-parity
> review that restored upstream charging-icon and killall semantics, ports of
> `OnionUI/Onion` PRs **#1936–#1946**, and the 2026-09-09 list-cache / installer / boot-FB
> / AXP-percent fixes. Measured on **`onionplus-compact`**.

**Rows are sorted by impact**, highest first: 🟥 vectorised / order-of-magnitude wins →
🟧 syscall & scan eliminations → 🟨 smaller structural wins → 🟦 robustness (no perf claim)
→ 🟩 quality floor. The second column also tags the **domain** touched:
⚡ CPU/algorithm · 🖼️ rendering · 🔋 power & idle wake-ups · ⚙️ process/syscall · 💾 storage & I/O ·
🏗️ build · 🛡️ robustness · 🧪 tests.
Commit SHAs are **not** repeated per row — every change is traceable in
[§1 Commit Breakdown](#-1-commit-breakdown) and in [§4.7](#47-pre-existing-defects-fixed-in-this-branch) / [§4.9](#49-game-switcher-review-pass-fixes-commits-fa888f22-d05267ca-ff012faa).

| 🎚️ | Category | Before | After | Δ | Evidence |
|:--:|:---------|:------:|:-----:|--:|:--------:|
...
| 🟥 | 🖼️ TTF/list/footer/header/dialog surface cache | `TTF_RenderUTF8_Blended` every frame | hash-invalidated SDL surfaces | 🚀 **5–15 ms/frame saved** | 📏 |
...
| 🟥 | ⚙️ GS overlay `playActivity` calls | `system("playActivity … &")` | double-fork + `execl` (async, no zombies) | 🚀 **−80 % process overhead** | 📏 |
| 🟧 | ⚙️ `mkdirs()` | `system("mkdir -p")` | direct `mkdir()` walk | **−100 % proc spawns** | 📐 🧪 |
| 🟧 | ⚙️ `file_copy()` | `system("cp -f")` | `open`/`read`/`write` loop | **−100 % proc spawns** | 📐 🧪 |
| 🟧 | ⚙️ `config.h` `_config_prepare` | `system("mkdir -p …")` | direct `mkdirs()` walk | **−100 % proc spawns** | 📐 🧪 |
| 🟧 | ⚙️ GS overlay RetroArch kill/poll | `killall` / `pidof` shell-outs | `process_killall_signal` / `process_isRunning` | **−100 % shell, all PIDs** | 📐 |
| 🟦 | 🛡️ `battery_hasChanged` while charging | percBat could replace sentinel `500` | early-return like `OnionUI/Onion:main` | charging icon stable | 🛡️ |
...
| 🟨 | 🏗️ Release binary size (`--gc-sections`) | default `-O0` sections kept | `-O2 -ffunction-sections -Wl,--gc-sections` | **−5–15 %** | 📏 |
| 🟨 | 💾 `file_read()` | `fopen`+`fseek`×2+`ftell` | `stat64`+`read()` loop | **−2 seeks, −1 copy** | 📐 🧪 |
| 🟨 | 🖼️ `display_readOrWriteBuffer` | per-pixel loop on every row | `memcpy` fast path for contiguous rows | **row copy vectorised** | 📐 |
| 🟨 | 🔋 OSD overlay draw loop | full-throttle redraw for the overlay's entire duration | 2 ms `msleep` per iteration + stats demoted to `printf_debug` | **overlay CPU burn capped** | 📐 |
| 🟦 | 🛡️ `currentGame()` NULL dereference sites | 3 callers dereferenced unchecked (`game_list_len == 0` → NULL) | NULL guards at all 3 sites | **crash on empty game list removed** | 🛡️ |
| 🟦 | 🛡️ `_isContentNameInInfo` left-boundary read | `*(found - 1)` read 1 byte before the buffer at position 0 | string start treated as valid left boundary | **OOB read removed** | 🛡️ |
| 🟦 | 🛡️ `screenshot_save()` VLA undefined behaviour | no guard on `g_display.width/height == 0`; a 0-size VLA is UB | dimension checks before the VLA declaration | **undefined behaviour removed** | 🛡️ 🧪 |
...
| 🟦 | 🛡️ `action_loadGame` slot bounds check | `< 0 && >= slot_count` — untriggerable dead code | condition changed to OR | **out-of-range slots rejected** | 🛡️ |
| 🟦 | 🛡️ `file_isLocked()` O_CREAT flag | dropped during the port, breaking the save-state loop in `gs_popMenu.h` | flag restored | **save-state wait loop works again** | 🛡️ 🧪 |
...
| 🟦 | 🛡️ playActivity fork+exec | `waitpid()` on the child blocked the UI thread | double-fork, detached grandchild reparented to init | **async semantics restored, no zombies** | 🛡️ |
| 🟦 | 🛡️ Dialog theme-render cache | surfaces never released on theme change; memory accumulation | `theme_renderDialog_cleanup()` added | **memory leak closed** | 🛡️ 🧪 |
| 🟦 | 🛡️ MULTIVALUE label cache color | cache keyed only on value, not color; disabled-state feedback broken | cache keyed on the `color+value` pair | **UX regression fixed** | 🛡️ 🧪 |
| 🟦 | 🛡️ `includeCJK()` UTF-8 validation | only the first continuation byte was checked | all 3 bytes of the sequence validated | **complete UTF-8 validation** | 🛡️ 🧪 |
| 🟦 | 🛡️ File I/O error reporting & consistency | `file_remove_recursive()` swallowed `nftw()` errors; `file_changeKeyValue()` had no fsync | errors logged; fsync before rename | **data consistency + error visibility** | 🛡️ 🧪 |
| 🟦 | 🛡️ `RUN_TEST` macro result reporting | printed `[ OK ]` for both pass **and** fail | prints `[FAIL]` for failures | **silently failing tests now visible** | 🛡️ 🧪 |
...
save-state path and the `const` cast ([§4.7](#47-pre-existing-defects-fixed-in-this-branch)),
plus the `currentGame()` NULL derefs, the dead slot check and the content-match OOB read
([§4.9](#49-game-switcher-review-pass-fixes-commits-fa888f22-d05267ca-ff012faa)). The
`file_isLocked` / `RUN_TEST` / dialog-cleanup trio and the MULTIVALUE / VLA / UTF-8 / fsync
set come from [PR #206](https://github.com/Amiga500/Onion/pull/206) and
[PR #207](https://github.com/Amiga500/Onion/pull/207); the blocking `waitpid` was a port
regression fixed at the tip.*
...
|:-:|:----|:--------|------:|----:|:-----|
| 1 | [`d7aed5a1`](https://github.com/Amiga500/Onion/commit/d7aed5a1b40f6d021a58a41589d6371b30a81142) | ⚡ NEON pixel conversions vs OnionUI scalar loops | 7 | +483 / −80 | perf |
...
| 3 | [`ad402fa2`](https://github.com/Amiga500/Onion/commit/ad402fa2e400af6538f971f89979ac0647daa98f) | 🛡️ Crash/memory hardening vs OnionUI common layer | 33 | +9,724 / −193 | hardening |
...
| 5 | [`300390a7`](https://github.com/Amiga500/Onion/commit/300390a7bfec1887acaa261cfd711b57c70c23fc) | 🧪 Host unit-test suite (absent on OnionUI/Onion:main) | 58 | +13,894 / −18 | test |
...
| 11 | [`4b851203`](https://github.com/Amiga500/Onion/commit/4b851203) | ⚡ OSD busy-wait fix vs OnionUI usleep(100) | 1 | +51 / −30 | perf |
| 12 | [`e8143d09`](https://github.com/Amiga500/Onion/commit/e8143d09) | ⚡ Brightness sysfs cache vs OnionUI uncached PWM | 1 | +40 / −13 | perf |
| 13 | [`0121f943`](https://github.com/Amiga500/Onion/commit/0121f943) | ⚡ Battery charging cache vs OnionUI uncached axp_test | 3 | +96 / −30 | perf + fix |
| 14 | [`0d1ce423`](https://github.com/Amiga500/Onion/commit/0d1ce423) | ⚡ SQLite open/close 2→1 vs OnionUI | 6 | +250 / −81 | perf + hardening |
...
| 32 | [`e17c6a9b`](https://github.com/Amiga500/Onion/commit/e17c6a9b) | 🛡️ Correct GameSwitcher romscreen capture stride and blit | 3 | +70 / −17 | fix |
| 33 | [`82fab865`](https://github.com/Amiga500/Onion/commit/82fab865) | 🛡️ GameSwitcher preview FB stride and stretch romscreens | 3 | +6 / −4 | fix |
| | | **Aggregate `07505ea5` → `82fab865` (code tip, excl. `docs/`)** | **142** | **+25,668 / −811** | |
| | | **Full range `07505ea5` → this docs refresh** | **144** | **+27,234 / −811** | |

> ℹ️ **`git rev-list --count 07505ea5..HEAD` is 56**, not the 33 milestone rows above.
> The extra 23 are: 3 merge commits (`f9bac9fc`, `55de00a9`, `e44421e1` is listed), 3 CI
> `clang-format` passes, 8 docs-only refreshes (including `971d6169` listed as #6 and
> `e0b6893c`), a 5-commit remote experiment (`eea25f88`…`4f7841e0`) that is **net zero** in
> the tree, the individual PR #206/#207 commits grouped as rows 18–19, and the 2026-08-22
> version bump. A previous revision of this document advertised **52** commits at tip
> `ddbb7e14`; that was stale after the GameSwitcher stride fixes.
>
...
>
> **Timeline:**
> - **Commits 1–10**: NEON + hardening + test suite vs OnionUI/Onion:main.
...
> - **Commits 32–33**: 2026-08-24 GameSwitcher framebuffer stride / romscreen stretch fixes (`e17c6a9b`, `82fab865`).
>
...
> **+2,323 / −766 across 60 `src/` files**, i.e. **8.5 %** of the added lines. Everything else
...
> 📏 **180° rotation vs OnionUI software rotozoom:** **~2 ms → ~40 µs** with in-place `VREV64`
...
> Throughput is *not* the same thing as wall-clock speedup: on a memory-bound Cortex-A7
> the real gain is capped by SD-card and DRAM bandwidth. Values above are what the
> OnionPlus kernels actually implement vs OnionUI scalar loops.
...
> (📐 exact, structural). Speedup magnitude is 📏 OnionPlus vs OnionUI scalar `SDL_GetRGBA`.
...
> 🐞 **OnionPlus keeps the correct alpha scale here.** An earlier NEON draft multiplied by the
...
- ❌ **No on-device re-bench on a physical Miyoo in this tree.** 📏 still means OnionPlus vs OnionUI/Onion:main.
...
> 📏 **−90 %** vs OnionUI `strlen()`-in-the-loop for long strings.
...
> 📏 **−50 %** string scans vs OnionUI `strlen`+`strcpy`. 🧪 `test_file` (88 / 183).
...
> 📏 **−80 %** process overhead vs OnionUI `system("playActivity … &")` after `fork`+`exec`
...
releases them. The render-side code that **populates** those fields now lives in
...
> 📏 **5–15 ms/frame saved** vs OnionUI `TTF_RenderUTF8_Blended` every frame.
...
exists; magnitudes are 📏 OnionPlus vs OnionUI, or 📐 structural.
...
📏 vs OnionUI `usleep(100)`: idle CPU **~10 % → <1 %**. The commit also adds
...
`pwm0/duty_cycle` sysfs write when the value is unchanged (📏 vs OnionUI: **−100 % duplicate
...
single `open` → `sqlite3_exec` → `close` cycle instead of two (📏 vs OnionUI: **−50 % DB
...
now run via double-fork + `execl` (📏 vs OnionUI `system()`: **−80 % process overhead**), and the
...
|:-----|:-------|:------|:-------|
| ⚡ `rumble()` GPIO init | OnionUI: `export` + `direction` on **every** vibration | one-time init + retry if gpio missing | 📐 −2 sysfs writes/pulse vs OnionUI |
| 🛡️ `settings.vibration` index | OnionUI: unclamped index — OOB if config > 3 | `CLAMP_VIBRATION()` to `VIBRATION_LEVEL_MAX` (3) | 🛡️ OOB read removed |
...
`07505ea5` vs the working tree. The full `src/` delta is 60 files; extra files (theme
...
| After 2026-09-01 A–G (long branch) | **68** | **1,414** | empty-file contract + Flip macros |
| After `bf3deb8e` (`onionplus-compact`) | **68** | **1,419** | `test_alpha_scale` +1 (list-cache dim); `test_battery` +4 (AXP clamp) |
...
Commit `ad402fa2` imported a `Makefile.unit` whose `TESTS` list still named all 67 earlier
...
and `-Wl,--gc-sections`, only when `DEBUG` is unset. These flags are
supported on the Miyoo GCC 8.3 toolchain. 📏 vs OnionUI default sections: **−5–15 %** binary size for
...
(`unit-test-san`), and a `neon-arm` job that runs the NEON tests under qemu. Push trigger
is `onionplus-compact` (`22004cce`). `jpg2png` is not built in `make core`.
...
| 🚀 **TTF/list/footer/header/dialog cache** | 📏 **5–15 ms/frame** *(vs OnionUI per-frame TTF)* |
| 🚀 **OSD bar busy-wait** | 📏 **idle CPU ~10 % → <1 %** *(vs OnionUI `usleep(100)`)* |
| 🚀 **Brightness duplicate sysfs writes** | 📏 **−100 %** *(vs OnionUI uncached PWM)* |
| 🚀 **`battery_isCharging` subprocess spawns** | 📐 **~−99 %** *(2 s cache, MIYOO354)* |
| 🚀 **playActivity DB open/close cycles** | 📏 **2 → 1** *(−50 % DB I/O vs OnionUI)* |
...
| 🚀 **Release `--gc-sections`** | 📏 **−5–15 %** binary size *(vs OnionUI default sections)* |
...
## 🚫 8. Intentionally not taken

These items exist on a sibling experiment or on `v4.5-dev` and are **absent** from
OnionPlus on purpose. They are **not** counted in the totals. Comparison baseline
remains `OnionUI/Onion:main`.
...
### 📏 What the percentages compare

Every **📏** figure — the NEON speedup column in [§2.1](#21-kernel-table), the +5000 % rotation,
the −90 % `str_count_char`, the −50 % scan reductions — is the **OnionPlus vs OnionUI/Onion:main**
delta for that path,
...
- **Expect a "23 vs 56" discrepancy against older revisions of this document.** The
  headline is `git rev-list --count 07505ea5..HEAD` (**56** at HEAD; code tip `82fab865`).
...
|:--|:--|:--|:--|
| `battery_hasChanged` | set `500` then let `/tmp/percBat` overwrite it | early `return` while charging, like upstream | Mini+/Flip charging icon stayed on |
| RetroArch overlay exit | `process_kill` first matching PID | `process_killall` / `process_killall_signal` | `killall` semantics |
| `file_read` empty file | `NULL` | allocated `""` (upstream parity) | callers that distinguish empty vs missing |
| `rumble()` GPIO | latch init even if export failed | retry until `gpio48` exists | unexport / first-boot race |
| `findRomScreen` / chargingState / GS titles | `sprintf`/`strcpy` leftovers | `snprintf` + size arguments | no path overflow |
| TTF caches on exit | surfaces left until process death | `theme_renderHeader_cleanup` / dialog cleanup | leak on Tweaks/GS quit |
| `cpuclock` device gate | overly wide id test | allowlist `283` / `285` / `354` | Flip included, unknown ids excluded |
...
CI: `test.yml` runs on push to `onionplus-compact` (`22004cce`). Host `make unit-test` at
the 2026-09-01 long-branch tip was **1,414 tests**. Compact now **1,419** (see [§4.13](#413-2026-09-09-robcodedev-ports--review-fixes)).
`9ab47af` / `2f90bbe` carry the same E–G tree on the long branch; `fa5bb007` only added
the `OnionPlus` push trigger.

Not covered by that review: `docs/`, `website/`, `third-party/`, Flip lid/Hall on hardware, OTA mid-`install.sh` power cut.

---

## 🔀 4.13 2026-09-09: @robcodedev ports + review fixes
...
Still **open** upstream at the time of the port. Landed here via
[Amiga500/Onion #217](https://github.com/Amiga500/Onion/pull/217)
(`c7a1a7e9`, `587c35ec`, merge `f87e7781`). Copied onto compact — **not** authored by
`robcodedev` in this git history.

| Upstream PR | What landed |
|:--|:--|
| [#1936](https://github.com/OnionUI/Onion/pull/1936) | keymon: SELECT triggers a display refresh |
| [#1937](https://github.com/OnionUI/Onion/pull/1937) | `lt.lang` shipped as JSON |
| [#1938](https://github.com/OnionUI/Onion/pull/1938) | ThemeSwitcher: on-demand theme previews |
| [#1939](https://github.com/OnionUI/Onion/pull/1939) | GameSwitcher favorites + crash fixes |
| [#1940](https://github.com/OnionUI/Onion/pull/1940) | `fbmode` + framebuffer page transitions |
| [#1941](https://github.com/OnionUI/Onion/pull/1941) | `.forceKillRetroarch` |
| [#1942](https://github.com/OnionUI/Onion/pull/1942) | `romwinidx` stored on SD |
| [#1943](https://github.com/OnionUI/Onion/pull/1943) | theme selection keyed per `SERIAL_NUMBER` |
| [#1944](https://github.com/OnionUI/Onion/pull/1944) | recents list capped at 200 |
| [#1945](https://github.com/OnionUI/Onion/pull/1945) | skip RetroArch cfg patch when unchanged |
| [#1946](https://github.com/OnionUI/Onion/pull/1946) | overlap-launch preparation |

`c7a1a7e9`: 3 files · +88 / −16. `587c35ec`: 15 files · +1,326 / −112.

### Review fixes (not from those PRs)

| Commit | Area | What changed |
|:--|:--|:--|
| [`fbd26d06`](https://github.com/Amiga500/Onion/commit/fbd26d06) | List TTF cache | `_blit_cached_label()` dims a `SDL_ConvertSurface` copy. `surfaceSetAlpha` on the cache mutated pixels; restoring with 255 is identity. `test_alpha_scale`: `scale_alpha_255_does_not_undo_dim`. |
| [`fbd26d06`](https://github.com/Amiga500/Onion/commit/fbd26d06) | Installer Flip vs Plus | Hall-first (`hall-mh248`); `axp` / `axp_test` = Plus. Never `/dev/input/event*`. Framebuffer preclear before `check_device_model` (`fbmode` if present, else `dd` + `fbset 640x480/2`). |
| [`bf3deb8e`](https://github.com/Amiga500/Onion/commit/bf3deb8e) | Boot FB | Plus/Flip: dmesg `640x480` no longer skips the `mi_fb0` poll (avoids locking a 752 panel at 640). `commit_mainui_fbmode()` honors `wait_for_fb_driver`; on timeout uses `fbset`, not `fbmode` with the driver down. |
| [`bf3deb8e`](https://github.com/Amiga500/Onion/commit/bf3deb8e) | AXP `percBat` | `getBatPercMMP()` keeps last sane **0–100**; never writes `-1` or garbage (e.g. `1735289191`). `test_battery`: `axp_percent_*` clamp contract. |

`fbd26d06`: 3 files · +58 / −17. `bf3deb8e`: 3 files · +79 / −23. Net tests vs the 1,414 long-branch floor: **+5** (`test_alpha_scale` +1, `test_battery` +4).
...
| Mini Flip lid / Hall | hardware | Ported from `v4.5-dev` (`fd18bdd1` on compact / `921155e8` on the long branch), then finding C switched runtime detect to AXP then `hall-mh248` (not `/dev/input/event1`). Installer is **hall-first** (`fbd26d06`). Lid-already-closed no longer forces `deepsleep()` (finding B). **Still unvalidated on a physical Flip.** |
| Plus/Flip boot at 640 | closed | `bf3deb8e`: dmesg `640x480` no longer skips `mi_fb0`; `commit_mainui_fbmode` waits for the driver. |
| AXP `/tmp/percBat` garbage | closed | `bf3deb8e`: `getBatPercMMP()` clamps to last good 0–100. |
...
(now in tree), infoPanel scaled-image cache, GameSwitcher battery-poll throttle,
...
caches (list dimming no longer mutates the cache), `--gc-sections` release flags, four algorithmic wins in `str`/`file`, production
...
([§4.11](#411-onionui-parity-review)), the 2026-09-01 A–G review
([§4.12](#412-independent-review-2026-09-01-findings-ag)), ports of `OnionUI/Onion`
...
**OnionPlus vs `OnionUI/Onion:main`** figures, separated from **📐 analytical** counts.
They have not been re-timed on a physical Miyoo in this tree.
