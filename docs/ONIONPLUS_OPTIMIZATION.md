...
> **12 net commits + infra (4 reverted, net zero)** · **124 files** · **+26,154 / −535 lines** · **8 NEON kernels** · **67 test suites** · **1,407 tests** · **71,363 assertions** · **ALL PASSED** ✅
...
The full `src/` delta is now **42 files** (TTF cache, signal-handler call sites, `reset.h`,
`config.mk`, …).*
...
| 10 | [`eb3f0aec`](https://github.com/Amiga500/Onion/commit/eb3f0aec) | 🎨 Port TTF label cache, shared signal handlers, and leftover hardening *(lands the former WIP)* | 18 | *see aggregate* | cache + hardening |
| 11 | [PR#206](https://github.com/Amiga500/Onion/pull/206) | 🛡️ **CRITICAL**: restore `file_isLocked` O_CREAT, fix `RUN_TEST` [FAIL] reporting, add dialog cleanup | 4 | +58 / −15 | critical fix |
| 12 | [PR#207](https://github.com/Amiga500/Onion/pull/207) | 🛡️ **MEDIUM**: MULTIVALUE cache color, screenshot VLA guard, UTF-8 validation, fsync consistency | 6 | +42 / −28 | medium fix |
| — | [`2537c94d`](https://github.com/Amiga500/Onion/commit/2537c94d) | 🏗️ No `SDL_Color.a` on Miyoo toolchain (ARM build fix for PR#207 color key) | 1 | +4 / −4 | build fix |
| | | **Aggregate `07505ea5` → HEAD** | **124** | **+26,154 / −535** | |
| | | *Code only, excluding `docs/`* | *122* | *+24,886 / −535* | |

> ℹ️ **12 net commits: 9 original port + WIP landing (`eb3f0aec`) + 2 targeted bugfix PRs.**
...
>
> ⚠️ **Reverted commits.** `eea25f88`/`10ec2387`/`840e2f2a`/`1e359571` (`process_start`
> fork+execv rewrite, OSD changes) were reverted same-day in `4f7841e0`: the `execv` rewrite
> passed multi-word `args` as a single argv entry, breaking callers such as
> `cpuclock "0 60"`. Net contribution: **zero lines**.
...
`07505ea5` vs the working tree. The full `src/` delta is 42 files; extra files (theme
...
| Metric | Value |
|:-------|------:|
| 🔧 **Commits** | **12 net** *(9 original + `eb3f0aec` + PR#206-207)* + infra · 4 reverted (net zero) |
| 📁 **Files changed** | **124** *(122 excluding `docs/`)* |
| ➕ **Lines added / removed** | **+26,154 / −535** *(+24,886 / −535 excluding `docs/`)* |
| 🧩 **Production code (`src/`)** | **42 files · +1,601 / −520** |
| 🧪 **Test code (`test/`)** | **75 files · +23,233 / −10** |
| 📚 **Documentation (`docs/`)** | **2 files · +1,268** |
...
- **Expect a "12 vs N" discrepancy against GitHub release notes** that compare a tag range
  missing the earliest NEON commit or the docs commits. This report scopes to the full
  `07505ea5` → `HEAD` delta (reverted commits excluded — they net to zero).
...
1. 🟡 Replace `process.h` `system(cmd)` with `fork`/`execv` (the last shell-out in the original 25-file set).
   ⚠️ *Attempted 2026-08-22 and **reverted same-day** (`4f7841e0`): the rewrite passed multi-word
   `args` as a single argv entry. A correct version must tokenize `args` (or keep `/bin/sh -c`)
   and reap children (`SIGCHLD`/`waitpid`) when `await=false`.*
...
OnionPlus is **12 net commits** ahead of upstream `OnionUI/Onion:main`
...
Commits analyzed: **12 net + infra (4 reverted, net zero)** · Date: 2026-08-22</sub>
