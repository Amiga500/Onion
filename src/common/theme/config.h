...
Theme_s theme_loadFromPath(const char *theme_path, bool apply_overrides)
{
    ...
    snprintf(config_path, STR_MAX * 2, "%sconfig.json", config.path);
...
void theme_changeOverride(const char *group_key, const char *key, void *value,
                          int value_type)
{
    ...
    switch (value_type) {
    case cJSON_String:
        cJSON_AddStringToObject(group, key, (char *)value);
        break;
    case cJSON_Number:
        cJSON_AddNumberToObject(group, key, (double)(*(int *)value));
        break;
    case cJSON_True:
        cJSON_AddTrueToObject(group, key);
        break;
    case cJSON_False:
        cJSON_AddFalseToObject(group, key);
        break;
    default:
        break;
    }
...
void theme_changeOverrideFile(const char *group_key, const char *key,
...
bool theme_getOverride(const char *group_key, const char *key, void *out_value,
                       int value_type)
{
    cJSON *group;
    ...
    switch (value_type) {
    case cJSON_String:
        value = json_getString(group, key, (char *)out_value);
        break;
    case cJSON_Number:
        value = json_getInt(group, key, (int *)out_value);
        break;
    case cJSON_True:
    case cJSON_False:
        value = json_getBool(group, key, (bool *)out_value);
        break;
    default:
        break;
    }
