...
void switch_zoom_profile(int segment_duration)
{
    ...
    case 7200:
        ...
        snprintf(label[0], 4, "%s", "4h");
        snprintf(label[1], 4, "%s", "8h");
        snprintf(label[2], 4, "%s", "12h");
        snprintf(label[3], 4, "%s", "16h");
        break;
    case 3600:
        // A segemmt is 60 minutes
        snprintf(label[0], 4, "%s", "2h");
        snprintf(label[1], 4, "%s", "4h");
        snprintf(label[2], 4, "%s", "6h");
        snprintf(label[3], 4, "%s", "8h");
        break;
    case 1800:
        // A segemmt is 30 minutes
        snprintf(label[0], 4, "%s", "1h");
        snprintf(label[1], 4, "%s", "2h");
        snprintf(label[2], 4, "%s", "3h");
        snprintf(label[3], 4, "%s", "4h");
        break;

    default:
        snprintf(label[0], 4, "%s", "");
        snprintf(label[1], 4, "%s", "");
        snprintf(label[2], 4, "%s", "");
        snprintf(label[3], 4, "%s", "");
        break;
    }
}
        ...
        if (bat_log_db != NULL) {
            ...
            if (rc == SQLITE_OK) {
                ...
                while ((sqlite3_step(stmt) == SQLITE_ROW) && (!b_quit)) {
                    ...
                    if (total_duration == 0) {
                        snprintf(current_percentage, 10, "%d%%", bat_perc);
                    }
...
void renderPage()
{
    ...
    switch (current_zoom) {
    case 0:
        snprintf(sub_title, 30, "%s", "16 HOURS VIEW");
