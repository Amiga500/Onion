...
RomScreenType_e findRomScreen(const Game_s *game, char *currPicture)
{
    ...
    //     snprintf(currPicture, STR_MAX * 2, STATES_DIR "/%s/%s.state.auto.png", game->core_name, game->rom_name);
    ...
    // Check if artwork exists - use strncpy for simple string copy
    strncpy(currPicture, game->recentItem.imgpath, STR_MAX * 2 - 1);
    currPicture[STR_MAX * 2 - 1] = '\0';
